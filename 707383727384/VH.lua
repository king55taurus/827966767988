...
local ReplicatedStorage = game:GetService("ReplicatedStorage") or game:GetService("ReplicatedStorages")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local net = ReplicatedStorage:WaitForChild("Packages") :WaitForChild("_Index") :WaitForChild("sleitnick_net@0.2.0") :WaitForChild("net")

local VirtualUser = game:GetService("VirtualUser") or game:GetService("VirtualUsers")
...
if Player and VirtualUser then Player.Idled:Connect(function() pcall(function() VirtualUser:CaptureController(); VirtualUser:ClickButton2(Vector2.new()) VirtualUser:TouchButton2(Vector2.new()) end) end) end

task.spawn(function() if _G.XPBar then _G.XPBar.Enabled = true end end)
...
_G.TeleportService = game:GetService("TeleportService") or game:GetService("TeleportServices")
_G.PlaceId = game.PlaceId

local function AutoReconnect() while task.wait(3) do if not Players.LocalPlayer or not Players.LocalPlayer:IsDescendantOf(game) then _G.TeleportService:Teleport(_G.PlaceId) end end end
...
local HttpService = game:GetService("HttpService") or game:GetService("HttpsService")
...
_G.AUTO_MESSAGE = "Salam Kenal Dari Pemula 🙏🏻"
_G.NEWBIE_MESSAGE = "Hallo"
...
if _G.HideLocalChat and not _G.ChatHiddenHooked then _G.ChatHiddenHooked = true
if _G.TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then _G.TextChatService.MessageReceived:Connect(function(msg)
if msg.TextSource and msg.TextSource.UserId == _G.LocalPlayer.UserId then msg:Cancel() end end) else local chatEvents = _G.ReplicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
if chatEvents and chatEvents:FindFirstChild("OnMessageDoneFiltering") then chatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(data)
if data.FromSpeaker == _G.LocalPlayer.Name then return nil end end) end end end


function _G.SendChat(msg) task.spawn(function()
local successNew = pcall(function()
if _G.TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then local channel = _G.TextChatService.TextChannels.RBXGeneral
if channel then channel:SendAsync(msg) return end end end)

if not successNew then pcall(function()
...
local function checkStatus()
    ...
    if response == "UPDATE" then
        if not CheckData.kicked then
            ...
            LocalPlayer:Kick("VionixHub Update,Rejoin Ulang Dan Execute Lagi!.")
        end
    elseif response == "LATEST" then
        if not CheckData.notified then
            CheckData.notified = true
            warn("[VionixHub] Status: Latest Version")
        end
    else
        warn("[VionixHub] Status unknown:", response)
    end
end
...
WindUI:Popup({
    ...
    Icon = "crown",
    Content = [[
Thank you for using Vionix Hub.
Don't forget to Join Vionix Hub Community!
]],
    Buttons = {
...
firesignal(_G.REFishingStopped.OnClientEvent) end
...
_G.REFishCaught = ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RE/FishCaught"] and ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RE/FishCaughtAnimation"] and ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RE/FishCaughtEffect"]
...
_G.REObtainedNewFishNotification = ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RE/ObtainedNewFishNotification"] or ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net["RE/ObtainedNewFishNotifications"]
...
_G.RemoteFish = _G.RemotePackage["RE/ObtainedNewFishNotification"] or _G.RemotePackage["RE/ObtainedNewFishNotifications"]
_G.RemoteSell = _G.RemotePackage["RF/SellAllItem"] or _G.RemotePackage["RF/SellAllItems"]

_G.RemoteFish.OnClientEvent:Connect(function(_, _, data)
if _G.sellActive and data then _G.fishCounter += 1
...
if player == Players.LocalPlayer and FuncAutoFish.autofish5x then _G.StopRecastSpam() _G.stopSpam() end end)



local lastEventTime = tick()

task.spawn(function() while task.wait(0) do if _G.AutoFishHighQuality and FuncAutoFish.autofish5x and FuncAutoFish.REReplicateTextEffect then if tick() - lastEventTime > 0 then StopAutoFish5X() lastEventTime = tick() task.wait(0) StartAutoFish5X() end end end end)

local function approx(a, b, tolerance) return math.abs(a - b) <= (tolerance or 0) end

local function isColor(r, g, b, R, G, B) return approx(r, R) and approx(g, G) and approx(b, B) end

local BAD_COLORS = {COMMON    = {1, 0.980392, 0.964706},
...
_G.DelayFinish = _G.FishSec:Input({
    Title = "Delay Finish",
    Desc = [[High Rod = 1 Medium Rod = 1.5 - 1.7 Low Rod = 2 - 3]],
...
if not stuck then NotifyWarning("Please Input Valid Number") end
_G.STUCK_TIMEOUT = stuck end})

myConfig:Register("StuckDelay", _G.StuckDelay)

_G.HighFish = _G.FishSec:Toggle({
    Title = "Filter Fish Quality",
    Desc = "Skip Common, Uncommon, Rare, Epic",
...
-- Stop Fishing Button
_G.FishSec:Button({Title = "Stop Fishing", Locked = false, Justify = "Center", Icon = "", Callback = function() _G.StopFishing() _G.StopAutoFishing() RodIdle:Stop() _G.stopSpam() _G.StopRecastSpam() end})
...
-- [FAKE FISHING CATCH VISUAL]
_G.ReplicatedStorage = game:GetService("ReplicatedStorage") or game:GetService("ReplicatedStorages")
_G.Players = game:GetService("Players")
_G.HttpService = game:GetService("HttpService") or game:GetService("HttpsService")
_G.LocalPlayer = _G.Players.LocalPlayer
_G.FakeFishingState = {Enabled = false, LoopDelay = 0.1, StepDelay = 0.1, SelectedRarities = {}}
_G.Net = _G.ReplicatedStorage.Packages._Index["sleitnick_net@0.2.0"].net
_G.REBaitCastVisual = _G.Net["RE/BaitCast"]
_G.REBaitSpawned = _G.Net["RE/BaitSpawn"] or _G.Net["RE/BaitSpawned"]
_G.RECaughtFishVisual = _G.Net["RE/CaughtFish"]
_G.REFishCaught = _G.Net["RE/FishCaught"] or _G.Net["RE/FishCaughts"]
_G.REObtainedNewFishNotification = _G.Net["RE/ObtainedNewFishNotification"] or _G.Net["RE/ObtainedNewFishNotifications"]
_G.FishDB_List = {}
_G.FishDB_ByName = {}
_G.Area_FishMap = {}
...
local itemsFolder = _G.ReplicatedStorage:WaitForChild("Items")
    for _, module in pairs(itemsFolder:GetChildren()) do
        if module:IsA("ModuleScript") then
            ...
            if success and data.Data and data.Data.Type == "Fish" then local detectedTier = data.Data.Tier or data.Data.Rarity or "Common"
if type(detectedTier) == "number" then
                    local tierMap = { [1]="Common", [2]="Uncommon", [3]="Rare", [4]="Epic", [5]="Legendary", [6]="Mythic", [7]="SECRET"}
                    detectedTier = tierMap[detectedTier] end
